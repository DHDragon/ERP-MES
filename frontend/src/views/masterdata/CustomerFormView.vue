<template>
  <el-card shadow="never">
    <template #header>{{ isEdit ? '编辑客户' : '新增客户' }}</template>
...
const form = reactive<MdCustomer>({ customerCode: '', customerName: '' })
...
const save = async () => {
  saving.value = true
  try {
    await customerApi.save({ ...form, id: isEdit.value ? id.value : undefined })
    ElMessage.success('保存成功')
    router.push('/masterdata/customers')
