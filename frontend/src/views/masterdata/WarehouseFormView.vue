<template>
  <el-card shadow="never">
    <template #header>{{ isEdit ? '编辑仓库' : '新增仓库' }}</template>
...
const load = async () => {
  ...
  const res = await warehouseApi.detail(id.value)
...
const save = async () => {
  saving.value = true
  try {
    await warehouseApi.save({ ...form, id: isEdit.value ? id.value : undefined })
    ElMessage.success('保存成功')
    router.push('/masterdata/warehouses')
