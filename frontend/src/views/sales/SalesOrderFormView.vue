<template>
  <el-card shadow="never" v-loading="loading">
    <template #header>{{ isEdit ? '编辑销售订单' : '新增销售订单' }}</template>
    ...
    <el-table :data="lines" border style="margin-top:8px">
      ...
      <el-table-column label="物料ID" width="140">
        <template #default="{ row }"><el-input-number v-model="row.materialId" :min="1" /></template>
      </el-table-column>
      <el-table-column label="数量" width="140">
        <template #default="{ row }"><el-input-number v-model="row.orderQty" :min="0.001" :precision="3" /></template>
      </el-table-column>
...
const lines = ref<any[]>([{ materialId: 1, orderQty: 1, remark: '' }])
...
const load = async () => {
  ...
  try {
    ...
    lines.value = (res.data.lines || []).map((x:any) => ({ materialId: x.materialId, orderQty: Number(x.orderQty), remark: x.remark || '' }))
  } finally { loading.value = false }
}

const addLine = () => lines.value.push({ materialId: 1, orderQty: 1, remark: '' })
const removeLine = (index:number) => { lines.value.splice(index, 1); if (!lines.value.length) addLine() }

const save = async () => {
  if (!header.customerId) { ElMessage.warning('客户ID必填'); return }
  if (!lines.value.length) { ElMessage.warning('至少一行明细'); return }
  saving.value = true
  try {
    await salesOrderApi.save({ header: { ...header, id: isEdit.value ? id.value : undefined }, lines: lines.value })
