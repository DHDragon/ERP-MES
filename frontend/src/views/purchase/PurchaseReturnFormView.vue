<template>
  <el-card shadow="never" v-loading="loading">
    <template #header>{{ isEdit ? '编辑采购退货' : '新增采购退货' }}</template>
...
const header = reactive<any>({ supplierId: 1, bizDate: '', remark: '' })
const lines = ref<any[]>([{ materialId: 1, returnQty: 1, remark: '' }])

const load = async()=>{
  if(!isEdit.value) return
  loading.value = true
  try{
    const res = await purchaseReturnApi.detail(id.value)
    Object.assign(header, res.data.header)
    lines.value = (res.data.lines || []).map((x:any)=>({ materialId:x.materialId, returnQty:Number(x.returnQty), remark:x.remark||'' }))
  } finally { loading.value=false }
}

const addLine = ()=>lines.value.push({ materialId:1, returnQty:1, remark:'' })
const removeLine = (i:number)=>{ lines.value.splice(i,1); if(!lines.value.length) addLine() }

const save = async()=>{
  saving.value = true
  try{
    await purchaseReturnApi.save({ header: { ...header, id: isEdit.value ? id.value : undefined }, lines: lines.value })
