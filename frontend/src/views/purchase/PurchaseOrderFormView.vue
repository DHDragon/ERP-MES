<template>
  <el-card shadow="never" v-loading="loading">
    <template #header>{{ isEdit ? '编辑采购订单' : '新增采购订单' }}</template>

    <el-form :model="header" label-width="100px">
      <el-form-item label="申请ID"><el-input-number v-model="header.reqId" :min="1" style="width:100%" /></el-form-item>
    ...
    <el-table :data="lines" border style="margin-top:8px">
      ...
      <el-table-column label="申请行ID" width="120"><template #default="{ row }"><el-input-number v-model="row.reqLineId" :min="1" /></template></el-table-column>
      ...
      <el-table-column label="数量" width="140"><template #default="{ row }"><el-input-number v-model="row.orderQty" :min="0.001" :precision="3" /></template></el-table-column>
...
const header = reactive<any>({ reqId: 1, supplierId: 1, bizDate: '', remark: '' })
const lines = ref<any[]>([{ reqLineId: 1, materialId: 1, orderQty: 1 }])
...
const load = async()=>{
  ...
  try{
    ...
    lines.value = (res.data.lines || []).map((x:any)=>({ reqLineId:x.reqLineId, materialId:x.materialId, orderQty:Number(x.orderQty) }))
  } finally { loading.value=false }
}

const addLine = ()=>lines.value.push({ reqLineId:1, materialId:1, orderQty:1 })
const removeLine = (i:number)=>{ lines.value.splice(i,1); if(!lines.value.length) addLine() }

const save = async()=>{
  saving.value = true
  try{
    await purchaseOrderApi.save({ header: { ...header, id: isEdit.value ? id.value : undefined }, lines: lines.value })
